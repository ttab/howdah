...
| Superseded claims | §5's and §9's "a `Rekey` sweep replaces the wait" — it does not, and the sweep has since been **removed**. A stored session is sealed in two places and a sweep reaches only the row, so it could never shorten the wait before a retired key is dropped; it could, however, write a revoked token back over a live one if it raced a refresh. The wait is the maximum session age with a store or without it. Any mention of `Rekey` or `Rekeyer` below is dead — see [cookies.md §11](cookies.md#11-rolling-a-key-over) |
